...
export function useSpacePresence(userId: string) {
    ...
    spaceChannel.subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          ...
          await spaceChannel.track({ user_id: userId, is_reading: isReading });
        }
      });
    ...
    }, [userId, isReading]); // isReading 상태가 바뀔 때마다 내 상태를 다시 알림
    ...
    const toggleReadingStatus = async () => {
      ...
      // 변경된 상태를 채널에 알림
      await channel.track({ user_id: userId, is_reading: newReadingStatus });
    };
